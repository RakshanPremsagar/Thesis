...
//    if(retvalPer != UA_STATUSCODE_GOOD) {
//        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
//                     "Could not create periodic job for server register. StatusCode %s",
//                     UA_StatusCode_name(retvalPer));
...
//    UA_StatusCode retval3 = UA_Server_unregister_discovery(server, clientRegister);
//
//    if(retval != UA_STATUSCODE_GOOD)
//        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
//                     "Could not unregister server from discovery server. StatusCode %s",
//                     UA_StatusCode_name(retval3));
//
...
static void Routine_run(UA_Server *server, UA_Client *clientRegister,UA_ServerConfig *config,
		char *discovery_url,UA_String *caps,UA_UInt64 callbackId,char *status){
	//UA_Server_run_shutdown(server);
	...
	caps[1]= UA_String_fromChars(status);
	//main();
	//sleep(1);
	//ExecuteBusyInstance();
	 //retval = UA_Server_addPeriodicServerRegisterCallback(server, clientRegister, discovery_url,
	                                                         //10 * 60 * 1000, 500,&callbackId);
	//sleep(15);
...
int main(int argc, char **argv) {
    ...
    //printf("%s",caps[1]);
    caps[0] = UA_String_fromChars("Crane");
    if(count!=0)
    ...
    while(running== true){
			...
			if(time(0)>timestamp){
				...
				if(count==1 && isDone == false){
						   status="Busy";
						   Routine_run(server,clientRegister,config,discovery_url,caps,callbackId,status);
...
//
//static
//UA_Client *getRegisterClientwithNoSecurity(UA_EndpointDescription *endpointRegister) {
//    if(endpointRegister->securityMode == UA_MESSAGESECURITYMODE_NONE) {
//        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "Using LDS endpoint with security None");
//        UA_Client *client = UA_Client_new();
//        UA_ClientConfig_setDefault(UA_Client_getConfig(client));
...
//        return EXIT_FAILURE;
//    }
//    UA_Client *clientRegister = getRegisterClientwithNoSecurity(endpointRegister);
